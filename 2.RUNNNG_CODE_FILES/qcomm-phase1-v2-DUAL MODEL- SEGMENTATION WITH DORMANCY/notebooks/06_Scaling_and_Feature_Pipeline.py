...
# %%
print("Model A (clustering):", RFM_COLS)
print("Model B (dormancy)  :", MODEL_B_FEATURES)
assert not (set(RFM_COLS) | set(MODEL_B_FEATURES)) & set(FORBIDDEN)
assert "monetary_sum" not in RFM_COLS, "monetary_sum would double-count frequency in the distance metric"
for c in RFM_COLS + MODEL_B_FEATURES:
...
save_json_s3({"rfm_cols": RFM_COLS, "model_b_features": MODEL_B_FEATURES,
