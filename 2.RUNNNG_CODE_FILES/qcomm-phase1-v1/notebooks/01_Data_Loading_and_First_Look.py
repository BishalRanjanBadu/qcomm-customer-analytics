...
#   Protected-attribute exclusion is therefore trivially satisfied for the clustering.
...
contract = {
    ...
    "clustering_spec": {"features": RFM_COLS, "must_drop": FORBIDDEN,
                            "permitted_features": RFM_COLS,
                            "profile_only": PROFILE_COLS,
    ...
    "tables": {
        "qcomm_customers_rfm": {"grain": "one row per customer", "primary_key": "customer_id",
            "rows": int(len(cust)),
            "columns": [_col(c, cust[c], c in RFM_COLS) for c in cust.columns]},
